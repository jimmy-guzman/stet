...
- The keymap is `createKeyHandler(ctx)` (`ctx` carries only `quit`); it reads/writes `state` directly and wraps each dispatch in Solid `batch` so a keypress is one update. Mouse is wired symmetrically: row components carry `onMouseDown` handlers (closures over their own row) that dispatch into the same `state` signals, each in `batch`, so a click mirrors the equivalent key. `quit` is the lone action kept in `ctx` because it needs the renderer; every other action, including `switchWorktree`, lives in `state` so the keymap, the worktree picker's click, and App's deleted-worktree recovery all reach it directly rather than through prop drilling. `quit` calls `renderer.destroy()` then `process.exit(0)`; the background poll keeps the event loop alive otherwise. Every quit trigger routes through this one `quit`: `q`/`esc` and ctrl-c alike. `render` sets `exitOnCtrlC: false` and the keymap handles ctrl-c itself, because OpenTUI's built-in `exitOnCtrlC` only calls `destroy()` (never `process.exit`), so it hangs behind the poll instead of exiting.
