...
- User config lives in `src/config/` (a normal domain: pure `paths`/`schema`/`load` plus a `Config` service), read from `~/.config/sideye/config.json` (JSONC) once at startup. It loads on its own short-lived `ManagedRuntime.make(ConfigLive)` in `main.tsx` _before_ the app runtime's first use, because the active theme must be set before the diff highlighter warm-up reads it; a missing/invalid config falls back to defaults plus a notice and never blocks startup. Named themes resolve through `src/theme/registry.ts` (`resolveThemes` validates each entry, applying a `{ base, ...overrides }` merge with cycle detection; `themeForName` is the lookup with a dark fallback) and `selectThemeName` picks the active one from the config selection and the detected appearance. The active name is held in `theme/mode.ts` and is static for the session in v1; a reactive runtime switch (and `theme/shiki.ts` re-registration) is the follow-up that lands #101.
