...
- **Viewer navigation is tabs over a pure history model.** `src/viewer/navigation.ts` is a pure module (no Solid/Effect/OpenTUI) holding `NavState` (tabs, each an ordered history of `Location`s, plus a per-path MRU viewport); it unit-tests like `git/tree`. `state` keeps the live signals (`selectedPath`/`cursorIndex`/`viewerScrollTop`/`viewerScrollX`) as the rendering source of truth and uses `navState` as an orthogonal **recorder**: a navigation captures the leaving `Location` into the active tab + MRU (`captureCurrent`/`recordCurrent`/`remember`), then applies the arriving one by setting the live signals plus a single `pendingRestore` that the `Viewer` effect consumes once the diff has loaded under the matching view intent. That one path replaced the old scattered resets, so "reset on file switch" is just restore-to-default. **Invariant:** at most one `preview` tab; all navigation routes to it (browse coalesces in place, a jump pushes), `ctrl-t` toggles pin/unpin, and navigating to a file already in a pinned tab focuses that tab (never duplicates). `selectFile`/`moveFocus` and the tab actions (`togglePinActiveTab`/`closeActiveTab`/`cycleTab`/`activateTab`) all live in `state`, reached by keymap/mouse/effects like every other action; cursor is stored as a file line (stable across refreshes), scroll is best-effort.
