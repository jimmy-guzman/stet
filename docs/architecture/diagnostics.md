...
A file's diagnostic badge moves through explicit states:

```mermaid
stateDiagram-v2
    [*] --> pending: file opened or changed
    pending --> clean: server published an empty set
    pending --> findings: server published diagnostics
    pending --> failed: server crashed
    pending --> unavailable: no server, cannot start, or download disabled
    clean --> pending: file changes
    findings --> pending: file changes
```
