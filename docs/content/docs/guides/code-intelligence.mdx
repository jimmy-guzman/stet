...
checks finish: type errors from TypeScript, lint findings from oxlint, Biome
where it is configured, and schema-aware JSON and YAML validation. Each is
tagged with its source and pinpointed to its `line:col`. Press `p` to open the
panel and `enter` to jump to a finding.
