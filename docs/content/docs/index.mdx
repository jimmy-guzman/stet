...
compares against the empty tree instead and every file reads as added. The scope
shows `uncommitted vs no commits yet` until the first commit, after which it
compares against `HEAD` like any other repository.
