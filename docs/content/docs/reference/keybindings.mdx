...
unknown id, a combo that does not parse, or a combo already taken by another
action reports a notice at startup and keeps the default; `?` and `--help`
always show the bindings actually in effect. Keys inside overlays (pickers,
menus, the quit confirm, the search pane) and `ctrl+c` are fixed.
