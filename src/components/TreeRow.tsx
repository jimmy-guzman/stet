...
export function TreeRow(props: { row: FileTreeRow }) {
  ...
  const focused = () => state.focusedPane() === "tree" && props.row.index === state.focusedRowIndex()
  ...
  if (node.type === "directory") {
    ...
    const hasBadges = () => {
      ...
      return (
        !isExpanded() &&
        (node.changedCount > 0 || Boolean(s?.failed) || Boolean(s?.pending) || (s?.errors ?? 0) > 0 || (s?.warnings ?? 0) > 0)
      )
    }
    ...
    return (
      ...
      >
        <box flexDirection="row">
          <text fg={nameFg()}>{`${indent}${isExpanded() ? "▾" : "▸"} ${truncateName(`${node.name}/`, maxNameLen())}`}</text>
          <RecencyDot level={recency()} />
        </box>
        <box flexDirection="row">
          {summary()?.failed ? <text fg={theme.colors.severity.error}>fail </text> : null}
          {(summary()?.errors ?? 0) > 0 ? <text fg={theme.colors.severity.error}>{`✖${summary()?.errors} `}</text> : null}
          ...
          {node.changedCount > 0 ? (
            <text fg={node.stage !== undefined ? theme.colors.stage[node.stage] : theme.colors.text.muted}>
...
export function RecencyDot(props: { level: RecencyLevel }) {
  ...
  return props.level === "none" ? null : (
    <text fg={props.level === "fresh" ? theme.colors.accent.primary : theme.colors.accent.dim}> ●</text>
  )
}
