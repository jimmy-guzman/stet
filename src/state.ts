...
function loadDiffView(src: {
  ...
  if (src.showFile) {
    const gitSpec =
      src.file?.kind === "deleted" ? (src.scope.kind === "unstaged" ? `:${src.path}` : `${src.scope.ref}:${src.path}`) : undefined
    return File.pipe(
      Effect.flatMap((file) => file.content(src.model.repoRoot, src.path, { full: src.full, gitSpec })),
      Effect.map(
        (content): DiffView => ({
  ...
  const file = src.file
  if (file === undefined) {
    return Effect.succeed<DiffView>({ diff: "", fileContent: undefined, path: src.path, showFileContent: false })
  }

  return Git.pipe(
    Effect.flatMap((git) => git.fileDiff(src.model.repoRoot, src.scope, file)),
    Effect.map((diff): DiffView => ({ diff, fileContent: undefined, path: src.path, showFileContent: false })),
    Effect.catch(() => Effect.succeed<DiffView>({ diff: "", fileContent: undefined, path: src.path, showFileContent: false })),
  )
}
...
function createState() {
  ...
  const showFileContent = createMemo(() => selectedPath() !== undefined && (selectedFile() === undefined || fileView()))
  const tree = createMemo(() => buildFileTree(gitModel().repoFiles, gitModel().changedByPath, { changesOnly: changesOnly() }))
  ...
  const lineMap = createMemo(() => {
    const path = selectedPath()
    return path === undefined ? new Map<number, Diagnostic[]>() : findingsLineMap(path, checkerState())
  })
        ...
        if (fileState.status === "failed" && fileState.message !== undefined) {
          fileState.message
            ...
            .forEach((line, lineIndex) => {
              items.push({ checker, id: `failure-${checker}-${lineIndex}`, isFirst: lineIndex === 0, kind: "failure", line })
            })
          break
        }
      }
  ...
  const paletteResults = createMemo(() => {
    ...
    const allPaths = [...new Set([...model.repoFiles.map((file) => file.path), ...model.changedByPath.keys()])]
  ...
  const renderedPatch = createMemo(() => {
    ...
    return renderPatch(view.diff, { full: view.showFileContent || fullContentPaths().has(view.path), maxLines: 1600 })
  })
  ...
  const sidebarWidth = createMemo(() => (sidebarOpen() ? Math.max(34, Math.min(54, Math.floor(terminalWidth() * 0.34))) : 0))
  const paletteWidth = createMemo(() => Math.max(30, Math.min(70, terminalWidth() - 8)))
  const paletteLeft = createMemo(() => Math.max(0, Math.floor((terminalWidth() - paletteWidth()) / 2)))
  ...
  function loadModel(input: { repoRoot: string; scope: DiffScope }) {
    return runtime.runPromise(Git.pipe(Effect.flatMap((git) => git.loadModel(input.repoRoot, input.scope))))
  }
  ...
  createEffect(() => {
          ...
          Effect.tap((next) =>
            Effect.sync(() => {
              ...
              if (prev.repoRoot === root && prev.repoFilesKey !== next.repoFilesKey) {
                setGitModel({ ...prev, repoFiles: next.repoFiles, repoFilesKey: next.repoFilesKey })
              }
            }),
          ),
          Effect.ignore,
    ...
    runtime.runPromise(Stream.merge(fast, slow).pipe(Stream.runDrain), { signal: controller.signal }).catch(() => {})
    onCleanup(() => controller.abort())
  })
